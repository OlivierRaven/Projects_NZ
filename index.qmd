...
# NZFFD
```{r}
nzffdms <- read_csv("data/data_raw/NZffd/nzffdms.csv")

nzffd_clean <- nzffdr_clean(nzffdms)
nzffd_clean <- nzffdr_add_dates(nzffd_clean)
nzffd_enhanced <- nzffdr_taxon_threat(nzffd_clean)

#nzffd_habitat <- nzffdr::nzffdr_widen_habitat(nzffd_enhanced,cols_to_expand = c("habitatFlowPercent", "habitatSubstratePercent"))
...
# LAWA data
```{r}
lawa_lake <- read_excel("data/data_raw/Lawa/lawa-lake-monitoring-data-2004-2023_statetrendtli-results_sep2024.xlsx",
...
metadata <- read.csv("data/data_raw/lernzmp/LERNZmp_lake_metadata.csv") %>%
  filter(aeme_file %in% lid_files)

aeme_list <- lapply(lid_files, function(lid) {
  readRDS(paste0("data/data_raw/lernzmp/", lid, ".rds"))
})
